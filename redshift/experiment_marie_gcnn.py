...
from backbone import GalaxyEquivariantMDN
...
class MarieGCNNHybrid(nn.Module):
    """Premieres couches G-CNN equivariantes + tete style Marie classification/regression."""

    def __init__(self, n_bins: int, meta_dim: int = 4) -> None:
        super().__init__()
        base = GalaxyEquivariantMDN(num_gaussians=3, use_meta=False)
        self.in_type = base.in_type
        self.block1 = base.block1
        self.block2 = base.block2
        self.block3 = base.block3
        self.block4 = base.block4
        self.gpool = base.gpool
        self.spatial_pool = nn.AdaptiveAvgPool2d(1)
        self.meta = nn.Sequential(nn.Linear(meta_dim, 64), nn.ReLU(), nn.Linear(64, 96), nn.ReLU())
        self.shared = nn.Sequential(nn.Linear(256 + 96, 256), nn.ReLU(), nn.Dropout(0.2), nn.Linear(256, 128), nn.ReLU())
    ...
    def forward(self, x: torch.Tensor, meta: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        h = enn.GeometricTensor(x, self.in_type)
        h = self.block1(h)
        h = self.block2(h)
        h = self.block3(h)
        h = self.block4(h)
        h = self.gpool(h).tensor
        img = torch.flatten(self.spatial_pool(h), 1)
        features = self.shared(torch.cat([img, self.meta(meta)], dim=1))
        return self.class_head(features), self.reg_head(features).squeeze(1)
...
def predict(model, loader, device, limit_batches: Optional[int]):
    ...
    with torch.no_grad():
        for batch_idx, (x, cond) in enumerate(loader):
            ...
            _, z_reg = model(x.to(device, non_blocking=True), meta_from_cond(cond_device))
...
def run(args: argparse.Namespace) -> None:
    ...
    model = MarieGCNNHybrid(n_bins=len(edges) - 1).to(device)
    ...
    torch.save(model.state_dict(), os.path.join(output_dir, f"marie_gcnn{suffix}.pt"))
    ...
    np.savez(os.path.join(output_dir, f"predictions_marie_gcnn{suffix}.npz"), z_true=z_true, z_pred=z_pred)
    write_rows_csv(os.path.join(output_dir, f"metrics_marie_gcnn{suffix}.csv"), [metrics])
