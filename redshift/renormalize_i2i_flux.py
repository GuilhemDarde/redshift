...
def renormalize_images(
    ...
    aug_flux = images_to_fluxes(x_aug)
    ...
    renorm_flux = images_to_fluxes(renorm)
...
def run(args: argparse.Namespace) -> None:
    ...
    fallback_flux = images_to_fluxes(dataset.data["x"][source_index])
    if args.target == "source_flux":
        target_flux, zero_points = source_flux_targets(dataset.data["x"], source_index)
